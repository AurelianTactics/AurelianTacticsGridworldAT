...
public class PlayerManager : Singleton<PlayerManager>
{
	...
	void UpdateTeamEnemyDict(int teamId, bool isAbleToFight)
	{
		if (isAbleToFight)
		{
			...
			{
				if (entry.Key.x == teamId)
				{
					if (sTeamIdToIsAbleToFight[entry.Key.y])
						sTeamEnemyDict[new Point(entry.Key.x, entry.Key.y)] = true;
				}
				else if (entry.Key.y == teamId)
				{
					if (sTeamIdToIsAbleToFight[entry.Key.x])
						sTeamEnemyDict[new Point(entry.Key.x, entry.Key.y)] = true;
				}
			}
		}
		else
		{
			foreach (KeyValuePair<Point, bool> entry in sTeamEnemyDict)
			{
				if (entry.Key.x == teamId || entry.Key.y == teamId)
				{
					sTeamEnemyDict[new Point(entry.Key.x, entry.Key.y)] = false;
